...
function PaymentCallbackContent() {
  ...
  useEffect(() => {
    if (!reference || !transactionId) {
      setMessage("Flutterwave did not return the payment details needed for verification.");
      setState("failed");
      return;
    }
    fetch(`/api/payments/flutterwave/verify?reference=${encodeURIComponent(reference)}&transactionId=${encodeURIComponent(transactionId)}`)
      .then(async (response) => {
  ...
  return <div className="mx-auto max-w-md py-16 text-center space-y-6">
    {state === "verifying" && <div className="space-y-3"><Loader2 className="h-12 w-12 animate-spin text-blue-600 mx-auto" /><h1 className="text-lg font-bold text-gray-900 dark:text-white">Verifying payment...</h1><p className="text-xs text-gray-500">Please wait while we confirm your Flutterwave transaction.</p></div>}
