...
export function FulfillmentActions({ orderId, status }: { orderId: string; status: "PROCESSING" | "SHIPPED" }) {
  ...
  return <div className="text-right"><button type="button" onClick={updateStatus} disabled={isUpdating} className="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-3 py-2 text-xs font-bold text-white hover:bg-blue-700 disabled:opacity-60"><Icon className="h-4 w-4" />{isUpdating ? "Updating..." : label}</button>{error && <p className="mt-2 text-xs text-red-600">{error}</p>}</div>;
}
